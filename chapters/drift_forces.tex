...
MDLHydroD considers the effect of hull emergence angle, hence the drift force is considerly
less compared to WAMIT6.
...
Expanding \ref{eq:per_force} and separating the terms with 
...
to $\epsilon^2$, zeroth, first and second order force can be obtained 
whose equations are given below :
\begin{align}     
    \boldsymbol{\vec{F}}^{(2)} &= -\int_{wl}\frac{1}{2}\rho g (\zeta_r^{(1)})^2 \frac{\vec{\eta}^{(0)}}{\sqrt{1-\eta_3^2}} dl + 
